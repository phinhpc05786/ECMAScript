  ...
  .then(function (response) {
    ...
    function fetchProductDetails(productId) {
      return fetch(`http://localhost:3000/products/${productId}`)
        .then(response => {
          if (!response.ok) {
            throw new Error(`Không thể lấy thông tin sản phẩm cho ID ${productId}: ${response.statusText}`);
          }
          return response.json();
        })
    ...
    .then(categoryNames => {
      return Promise.all(array.map(element => fetchProductDetails(element.id)))
      .then(productDetails => {
